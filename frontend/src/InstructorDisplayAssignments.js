...
class InstructorDisplayAssignments extends Component {
    ...
    async handleSubmit(event) {
        ...
        const response = await fetch('http://localhost:8080/api/Assessment/instructor/' + item.net_id);
    ...
    render() {
        const {item} = this.state;
        const title = <h2>{'Display Assignments By Instructor'}</h2>;
...
export default withRouter(InstructorDisplayAssignments);